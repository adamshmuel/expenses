...
## New finding, still open (this pass)

| ID(s) | What | Root cause | Impact |
|---|---|---|---|
| ER-10, ER-11, DJ-01 | `GET /expenses` and `GET /expenses/summary` both silently drop an expense recorded **today** whenever the request includes `to=<today>` | A second, separate instance of the same "raw date/aggregate comparison isn't cast correctly" bug class that ER-08/09 caught for `ObjectId`s — this time for dates. `queryExpenses` (`Model.find()`) auto-casts the bare `"YYYY-MM-DD"` string to **midnight UTC that day**, so `$lte` excludes anything recorded later the same day. `getExpenseTotalByCategory` (raw `.aggregate()`) doesn't cast at all, so a string `to` never matches a BSON `Date`. Confirmed independently with a plain script against backend's own code. | The Dashboard's own "This month" period always sends `to = today`, so **both** `GET /expenses` and `GET /expenses/summary` silently exclude today's spending — `DashboardPage.tsx` under-reports (or, if all of today's data, still shows its empty state) for any user who spent today. Reproduced end-to-end in DJ-01 (`qa/e2e/tests/chat-dashboard-journey.spec.ts`), which now fails for this reason instead of the (now-fixed) ObjectId bug. Ship-blocking for `02-dashboard.md`, same severity class as the fixed bug. |

All of the above are in `backend/` outside `ai/` — Adam's own code; QA reports, does not fix. See `qa/specs/api-expenses-categories.md` and the run report for full detail and a suggested fix.
